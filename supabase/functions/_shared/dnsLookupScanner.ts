...
async function queryDoH(
  ...
  try {
    ...
    return {
      ...
      error:
        body.Status && body.Status !== 0 && body.Status !== 3
          ? `DNS status ${body.Status}${body.Comment ? `: ${body.Comment}` : ""}`
          : undefined,
      durationMs: Date.now() - started,
    };
  } catch (error) {
...
export async function scanDnsLookup(options: DnsLookupOptions): Promise<DnsLookupResult> {
  ...
  const primaryTasks = recordTypes.map((type) => async () =>
    queryDoH(normalizedDomain, type, resolverUrl, timeoutMs, options.userAgent)
  );

  const primarySets = await runLimited(5, primaryTasks);
  const records: Record<string, DnsRecordSet> = Object.fromEntries(primarySets.map((set) => [set.type, set]));
...
function evaluateDnsPosture(
  ...
  const exposedTxt = txt
    .map((r) => r.data)
    .filter((value) =>
      /verification|site-verification|MS=|apple-domain-verification|atlassian-domain-verification|facebook-domain-verification/i.test(
        value,
      )
    );
