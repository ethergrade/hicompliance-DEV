...
serve(async (req: Request) => {
  ...
  try {
    ...
    const allowClearSensitiveInReport = reportMode === 'extended' && (
      isInternal || Boolean(
        callerProfile && canViewExtendedSensitiveData(callerProfile, customerId, darkRiskCapabilities)
      )
    );
    ...
    const sensitiveHitFields = allowClearSensitiveInReport
      ? 'source_record_id, source, source_label, query_kind, query_term, asset_scope, tag, masked_value, clear_value, match_policy, extraction_confidence, evidence_scope'
      : 'source_record_id, source, source_label, query_kind, query_term, asset_scope, tag, masked_value, match_policy, extraction_confidence, evidence_scope';

    const sourceRecordsQuery = adminClient
    ...
    if (reportMode === 'extended') {
      sourceRecordsQuery.eq('source_bucket', 'leaks.private.general');
    }
    ...
    const permittedExtendedSourceRecordIds = new Set(
      sourceRows
        .filter((row) => row.source_bucket === 'leaks.private.general')
        .map((row) => row.id),
    );
    const dtiSensitiveHits = ((dtiSensitiveHitsRes.data || []) as unknown as DtiSensitiveHitRow[])
      .filter((hit) => reportMode !== 'extended' || (
        Boolean(hit.source_record_id) && permittedExtendedSourceRecordIds.has(String(hit.source_record_id))
      ))
    ...
    const dtiSensitiveSamples = dtiSensitiveHits
      .map((hit) => {
        const tag = normalizeSensitiveTag(hit.tag);
        if (!tag) return null;
        const clearValue = allowClearSensitiveInReport
          ? safeText(String(hit.clear_value || ''), 180)
          : '';
