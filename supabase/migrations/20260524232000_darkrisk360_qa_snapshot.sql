-- DarkRisk360 MD10 QA snapshot helper
-- Provides a compact, machine-readable security QA view per customer.
...
DECLARE
  ...
  v_sensitive_customer_visible integer := 0;
...
BEGIN
  ...
  WHERE r.organization_id = _org_id
    AND (
      r.report_json::text ~* 'password\s*[=:]'
      OR r.report_json::text ~* 'token\s*[=:]'
      OR r.report_json::text ~* 'cookie\s*[=:]'
      OR r.report_json::text ~* 'sb_secret_[a-z0-9._-]+'
      OR r.report_json::text ~* 'sk-proj-[a-z0-9._-]+'
    );

  SELECT COUNT(*)
  INTO v_sensitive_customer_visible
  FROM public.darkrisk_evidence e
  WHERE e.organization_id = _org_id
    AND e.visibility = 'customer'::public.darkrisk_visibility
    AND COALESCE(e.contains_sensitive_data, false) = true;
  ...
  FROM public.darkrisk_audit_log a
  WHERE a.organization_id = _org_id
    AND a.action IN ('darkrisk_raw_evidence_revealed', 'darkrisk_raw_evidence_reveal_denied');
  ...
  RETURN jsonb_build_object(
    ...
    'checks', jsonb_build_object(
      ...
      'no_sensitive_customer_evidence', (v_sensitive_customer_visible = 0),
    ...
    'counts', jsonb_build_object(
      ...
      'sensitive_customer_visible', v_sensitive_customer_visible,
