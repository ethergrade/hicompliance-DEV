...
const isDarkRiskReport = (report: SurfaceScan360Report): boolean =>
	/darkrisk360/i.test(getBrandTitle(report)) ||
	/darkrisk360/i.test(String(report?.scan?.scan_profile || ""));
...
const collectScopeValues = (report: SurfaceScan360Report): string[] => {
	...
	const assets = Array.isArray(report.assets_in_scope)
		? report.assets_in_scope
		: [];
	const monitored = Array.isArray(report.monitored_scope)
		? report.monitored_scope
		: [];
	const scanTargets = Array.isArray(scan.scope_targets)
		? scan.scope_targets
		: [];
	...
	const values = [
		...
		...scanTargets.map((entry: unknown) =>
			typeof entry === "string"
				? entry
				: entry?.target || entry?.value || entry?.asset_value,
		),
	];

	const target = String(scan.target || "").trim();
	if (target && !/^scope completo|^scope cliente/i.test(target))
		values.push(target);
	return uniq(
		values.map((value) =>
			String(value || "")
...
const buildSimpleTable = (headers: string[], rows: string[][]): Table =>
	new Table({
				...
				children: headers.map(
					(headerLabel) =>
						new TableCell({
							children: [
								new Paragraph({
									children: [new TextRun({ text: headerLabel, bold: true })],
								}),
							],
...
const buildDocumentInfoRows = (report: SurfaceScan360Report): string[][] => {
	...
	return [
		[
			...
			darkRisk
				? "DarkRisk360 Estesa - Domain Threat Intelligence"
				: "SurfaceScan360",
		],
		[
			"Tipologia Documento",
			darkRisk
				? "Relazione Domain Threat Intelligence"
				: "Relazione SurfaceScan360",
		],
		...
		[
			"Revisionato da",
			"Dipartimento R&D - Dipartimento Digital Transformation",
		],
		["Cliente", clientName],
	];
};
...
const buildScopeRows = (report: SurfaceScan360Report): string[][] => {
	...
	if (scopeValues.length === 0)
		return [["1", asText(report.scan?.target, "Scope cliente")]];
	return scopeValues
		.slice(0, 250)
		.map((value, index) => [String(index + 1), value]);
};

const buildTemplateFrontMatter = (
	report: SurfaceScan360Report,
): Array<Paragraph | Table> => {
	const darkRisk = isDarkRiskReport(report);
	const org = report.organization || {};
	const clientName = asText(org.legal_name || org.name, "CLIENTE");
	const serviceTitle = darkRisk
		? "Accordo di Servizio - DTI"
		: "Accordo di Servizio - SurfaceScan360";
	...
	return [
		...
		paragraph("Tra:", true),
		paragraph(
			'HiSolution s.r.l., con sede in Via della Canapiglia 5 - Vecchiano (PI) (di seguito "Fornitore")',
		),
		paragraph("e"),
		...
		paragraph(
			"I servizi della Business Unit CyberSecurity di HiSolution utilizzano i seguenti standard e framework.",
		),
		paragraph(
			"CWE - Il Common Weakness Enumeration e un sistema di classificazione delle debolezze e delle vulnerabilita del software.",
			true,
		),
		paragraph(
			"CVE - Il sistema Common Vulnerabilities and Exposures fornisce un metodo di riferimento per vulnerabilita ed esposizioni di sicurezza informatica di pubblica conoscenza.",
			true,
		),
		paragraph(
			"CVSS - Il Common Vulnerability Scoring System e uno standard industriale aperto per la valutazione della gravita delle vulnerabilita.",
			true,
		),
		paragraph(
			"OWASP - L OWASP Top 10 evidenzia le principali criticita in ambito di sicurezza delle applicazioni web e supporta la prioritizzazione dei rischi.",
			true,
		),
		heading("Perimetro concordato", HeadingLevel.HEADING_1),
		paragraph(
			"Di seguito gli indirizzi URL, domini, sottodomini e IP concordati oggetto dello Scope of Work.",
		),
...
export async function generateSurfaceScan360Docx(
	...
	const cveCatalog = Array.isArray(report.cve_catalog)
		? report.cve_catalog
		: [];
	const recommendations = Array.isArray(report.ai?.top_recommendations)
	...
	const scanRows = [
		...
		[
			...
			asText(
				scan.started_at
					? new Date(scan.started_at).toLocaleString("it-IT")
					: "-",
			),
		],
		[
			...
			asText(
				scan.completed_at
					? new Date(scan.completed_at).toLocaleString("it-IT")
					: "-",
			),
		],
	];
