...
const isIpv4 = (value: string): boolean =>
	/^(?:\d{1,3}\.){3}\d{1,3}$/.test(String(value || "").trim());
...
export function adaptDarkRiskReportToSurfaceScanTemplate(
	reportJson: unknown,
	...
	const scopeAssets = Array.isArray(r?.scope?.authorized_assets)
		? r.scope.authorized_assets
		: [];
	...
	const reportBrandTitle = hasHiCompliance
		? "HICOMPLIANCE · DARKRISK360"
		: "HiConsole - DARKRISK360";
	const targetLabel =
	...
	const surfaceFindings = findings.map((finding: Record<string, unknown>) => {
		const cve = extractCves(
			...
			...(Array.isArray(finding?.evidence_summary)
				? (finding.evidence_summary as unknown[]).map(String)
				: []),
		);
		return {
			provider: "darkrisk360",
			module:
				String(finding?.type || "darkrisk")
					.trim()
					.toLowerCase() || "darkrisk",
			finding_type: String(finding?.type || "darkrisk_signal")
				.trim()
				.toLowerCase(),
			...
			affected_url: null,
			ip: isIpv4(String(finding?.affected_asset || "").trim())
				? String(finding?.affected_asset || "").trim()
				: null,
			port: null,
			protocol: null,
			remediation: String(
				finding?.interpretation ||
					"Gestire il finding con remediation tracciata e verifica successiva.",
			),
			cve,
			cwe: [],
			cvss: Number.isFinite(Number(finding?.risk_score))
				? Number(finding.risk_score) / 10
				: null,
			evidence: {
				summary: Array.isArray(finding?.evidence_summary)
					? (finding.evidence_summary as unknown[])
					: [],
			},
			...
			created_at:
				(finding?.last_seen_at as string) ||
				(finding?.first_seen_at as string) ||
				r?.generated_at,
		};
	});
	...
	const cveCatalog = Array.from(
		new Set(
			surfaceFindings.flatMap((entry: unknown) =>
				Array.isArray((entry as { cve?: unknown[] }).cve)
					? ((entry as { cve?: unknown[] }).cve as unknown[])
					: [],
			),
		),
	).map((cveId: unknown) => ({
		...
		cisa_kev: false,
		affected_assets: surfaceFindings
			.filter((entry: Record<string, unknown>) =>
				((entry.cve as unknown[]) || []).includes(cveId),
			)
			.map((entry: Record<string, unknown>) => entry.affected_asset as string)
			.filter(Boolean),
	}));
	...
	return {
		...
		scan: {
			...
			scope_jobs_total: 1,
			scope_job_ids: (reportJson as { scan_run_id?: string })?.scan_run_id
				? [(reportJson as { scan_run_id?: string }).scan_run_id as string]
				: [],
			scope_targets_total: scopeAssets.length,
			scope_targets: scopeAssets,
			scope_target_types: Array.from(
				new Set(scopeAssets.map((asset: string) => inferAssetType(asset))),
			),
		...
		assets_in_scope: scopeAssets.map((assetValue: string) => ({
			...
			hostname:
				inferAssetType(assetValue) === "domain" ||
				inferAssetType(assetValue) === "subdomain"
					? assetValue
					: null,
		...
		scope_guard_summary: {
			...
			excluded_by_scope: Array.isArray(
				r?.scope &&
					(r.scope as { excluded_assets?: unknown[] }).excluded_assets,
			)
				? ((r.scope as { excluded_assets?: unknown[] }).excluded_assets
						?.length ?? 0)
				: 0,
		...
		ai: {
			executive_summary:
				(reportJson as { executive_summary?: { text?: string } })
					?.executive_summary?.text || "",
			risk_score:
				(reportJson as { executive_summary?: { risk_score?: number } })
					?.executive_summary?.risk_score ?? null,
			risk_level:
				(reportJson as { executive_summary?: { risk_level?: string } })
					?.executive_summary?.risk_level || null,
			top_recommendations: recommendations
				...
				.map((entry: unknown, index: number) => {
					...
					return {
						...
						action: Array.isArray(e?.actions)
							? (e.actions as unknown[]).join("; ")
							: String(e?.actions || ""),
						affected_assets: Array.isArray(scopeAssets)
							? scopeAssets.slice(0, 10)
							: [],
						severity:
							e?.priority === "immediate"
								? "critical"
								: e?.priority === "short_term"
									? "high"
									: e?.priority === "mid_term"
										? "medium"
										: "low",
			...
			correlations: Array.isArray(
				(reportJson as { executive_summary?: { top_drivers?: unknown[] } })
					?.executive_summary?.top_drivers,
			)
				? ((
						(reportJson as { executive_summary?: { top_drivers?: unknown[] } })
							.executive_summary?.top_drivers ?? []
				...
				: [],
			compliance_notes:
				"Analisi DarkRisk360 normalizzata su template SurfaceScan360 per reporting unificato.",
		},
		ai_error: null,
	};
}
