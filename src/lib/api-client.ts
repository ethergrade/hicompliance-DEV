...
const configuredComplianceBaseUrl =
	(import.meta.env.VITE_COMPLIANCE_API_BASE_URL as string | undefined)?.replace(
		/\/$/,
		"",
	) ?? "";

// Override baseUrl se il dominio contiene websoupcloud
const isWebsoupCloud =
	typeof window !== "undefined" &&
	window.location.hostname.includes("websoupcloud");
...
export const API_BASE_URL = import.meta.env.DEV
	...
	: isWebsoupCloud
		? websoupCloudBaseUrl
		: configuredBaseUrl || "https://hiconsole.hisolution.it/api";
...
async function request<T>(
	...
	if (!response.ok) {
		...
		if (
			response.status === 403 &&
			(body as unknown)?.mfa_setup_required === true
		) {
			if (
				typeof window !== "undefined" &&
				window.location.pathname !== "/auth/mfa-setup"
			) {
	...
	if (!isJson) {
		...
		console.warn(
			`[api-client] Non-JSON 200 response from ${path}: ${text.substring(0, 200)}`,
		);
...
function createApiClient(baseUrl: string) {
	return {
		get<T>(
			...
			return request<T>(
				path,
				{ method: "GET", params, headers: opts?.headers },
				baseUrl,
			);
		},
		...
		post<T>(path: string, body?: unknown, opts?: ApiClientOptions): Promise<T> {
			return request<T>(
				path,
				{ method: "POST", body, headers: opts?.headers },
				baseUrl,
			);
		},
		...
		put<T>(path: string, body?: unknown, opts?: ApiClientOptions): Promise<T> {
			return request<T>(
				path,
				{ method: "PUT", body, headers: opts?.headers },
				baseUrl,
			);
		},
		...
		patch<T>(
			...
			return request<T>(
				path,
				{ method: "PATCH", body, headers: opts?.headers },
				baseUrl,
			);
		},
