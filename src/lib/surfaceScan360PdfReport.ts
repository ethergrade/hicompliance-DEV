...
const buildFallbackAi = (report: SurfaceScan360Report) => {
  ...
  const risk = report.exposure_score
    ? {
        score: report.exposure_score.posture_score,
        level: report.exposure_score.risk_level,
      }
    : fallbackRisk;
...
export function generateSurfaceScan360Pdf(report: SurfaceScan360Report): void {
  ...
  // 11. Exposure posture badge (right side)
  ...
  if (cveCatalogScoped.length > 0) {
    ...
    drawTable(['CVE', 'Stato', 'CVSS', 'EPSS', 'KEV', 'Asset', 'Descrizione'], cveRows, [72, 55, 36, 44, 32, 95, 181]);
  } else {
  ...
  if (aiData) {
    ...
    if (report.exposure_score) {
      const vulnerability = report.exposure_score.vulnerability_summary || {};
      text(
        `Punti rischio: ${report.exposure_score.risk_points}/100  ·  Metodo Exposure Score V${report.exposure_score.score_version || '3.0'}`,
        { size: 9, color: [MUTED.r, MUTED.g, MUTED.b] },
      );
      text(
        `Esposizioni operative: ${vulnerability.exposure_findings || 0}  ·  CVE confermate: ${vulnerability.confirmed || 0}  ·  CVE candidate: ${vulnerability.candidate || 0}  ·  Fingerprint da completare: ${vulnerability.fingerprint_unknown || 0}`,
        { size: 9, color: [MUTED.r, MUTED.g, MUTED.b] },
      );
      if (vulnerability.explanation) {
        text(redactReportWords(vulnerability.explanation), { size: 9 });
      }
    }
