...
export const hipatchApi = {
  ...
  async osPatchesPending(
    ...
    if (!id) return [];
    const params = date ? { date } : undefined;
    const res = await apiClient.get<{ success: boolean; data: HipatchOsPatch[] }>(
  ...
  async softwarePatchesPending(
    ...
    if (!id) return [];
    const params = date ? { date } : undefined;
    const res = await apiClient.get<{ success: boolean; data: HipatchSoftwarePatch[] }>(
  ...
  async remediations(
    ...
    if (!id) return [];
    const params = date ? { date } : undefined;
    const res = await apiClient.get<{ success: boolean; data: HipatchRemediation[] }>(
