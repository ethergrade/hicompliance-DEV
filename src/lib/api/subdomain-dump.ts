...
const groupHeader = (groupId: string) => ({ headers: { "X-Group-Id": groupId } });
...
function unwrap<T>(res: { success?: boolean; data?: T } | T[]): T[] {
  ...
  if (r && typeof r === "object" && "data" in r && r.data) return (r.data as unknown as T[]) ?? [];
  return [];
}
...
export const subdomainDumpApi = {
  ...
  async store(
    ...
    const res = await complianceApiClient.post<{ success: boolean; data: SubdomainDump }>(
   ...
   */
  async history(companyId: string, groupId?: string | null): Promise<SubdomainDump[]> {
    const res = await complianceApiClient.get<{ success: boolean; data: SubdomainDump[] }>(
