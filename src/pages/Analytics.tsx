...
const Analytics: React.FC = () => {
	...
	const { radarCategories, isLoading: trendsLoading } = useAssessmentTrends(
		assessmentId,
		groupId,
	);
				...
				<div className="flex items-center justify-between flex-wrap gap-3">
					<div>
						...
						<p className="text-muted-foreground">
							Monitoraggio dei trend di rischio, conformità NIS2 e minacce nel
							tempo
						</p>
					</div>
					...
					<Card className="border-border bg-card">
						<CardContent className="p-12 flex flex-col items-center justify-center text-center gap-3">
							...
							<p className="text-sm text-muted-foreground max-w-md">
								Non esiste ancora un assessment per l'azienda selezionata.
								Genera un assessment per visualizzare i trend di rischio,
									...
									<div className="flex items-center justify-between">
										<div>
											...
											<p className="text-2xl font-bold text-foreground">
												{riskScore > 0 ? riskScore : currentData.riskScore}
											</p>
											<div className="flex items-center mt-1">
												...
												<span
													className={`text-sm ${currentData.riskTrend === "up" ? "text-red-500" : "text-green-500"}`}
												>
													{currentData.riskTrend === "up" ? "+" : "-"}
							...
							<Card className="border-border bg-card">
								<CardContent className="p-4">
									<div className="flex items-center justify-between">
										<div>
											<p className="text-sm text-muted-foreground">
												Conformità Assessment Media
											</p>
											<p className="text-2xl font-bold text-foreground">
												{Math.round(
													assessmentComplianceData.reduce(
														(acc, cat) => acc + cat.compliance,
														0,
													) / assessmentComplianceData.length,
												)}
												%
											</p>
									...
									<div className="flex items-center justify-between">
										<div>
											...
											<div className="flex items-center mt-1">
												...
												<span
													className={`text-sm ${currentData.threatsTrend === "up" ? "text-red-500" : "text-green-500"}`}
												>
													{currentData.threatsTrend === "up" ? "+25%" : "-35%"}{" "}
													vs periodo prec.
												</span>
											</div>
										</div>
									...
									<div className="flex items-center justify-between">
										<div>
											...
											<Badge variant="default" className="mt-1 bg-green-600">
												{Math.round(
													(currentData.resolved / currentData.threats) * 100,
												)}
												% Risolte
											</Badge>
										</div>
