...
const SurfaceScan360: React.FC = () => {
  ...
  const scrollToDependencyMap = () => {
    if (isDiscoveryCollapsed) {
      ...
      setTimeout(() => {
        dependencyMapRef.current?.scrollIntoView({
          behavior: "smooth",
          block: "start",
        });
      }, 120);
  ...
  const scrollToExposureSection = () => {
    exposureSectionRef.current?.scrollIntoView({
      behavior: "smooth",
      block: "start",
    });
  };
    ...
    const dumpedSubdomains = (subdomainDump.history || []).flatMap(
      (dump: any) =>
        dump.results
          ...
          .filter(Boolean)
          .filter(
            (host) => !classifySurfaceHostForScope(host, scopeDomains).blocked,
          ),
    );

    const mergedSubdomains = [
      ...new Set([...discoveredSubdomains, ...dumpedSubdomains]),
    ];
  ...
  const excludedScannedTargets = useMemo(() => {
    return [...scanDiscovery.scannedTargets].filter(
      (target) => !visibleScannedTargets.includes(target),
    );
  ...
  const dumpedSubdomainMeta = useMemo(() => {
    ...
    for (const dump of subdomainDump.history) {
      for (const entry of dump.results) {
        ...
        const note = [entry.country, entry.asn_name]
          .filter(Boolean)
          .join(" · ");
        map[key] = {
          ip: entry.ip || null,
  ...
  const latestJobByHost = useMemo(() => {
    ...
    for (const job of scanJobs) {
      const host = String(job.hostname || "")
        .trim()
        .toLowerCase();
      if (!host) continue;
  ...
  const reverseAnalysisRows = useMemo(() => {
    ...
    for (const host of hostsToAnalyze) {
      ...
      const reverseHosts = [
        ...new Set(resolvedIps.flatMap((ip) => reverseDnsMap[ip] || [])),
      ];
  ...
  const domainIpDependencyGraph = useMemo(() => {
    ...
    for (const domain of candidateDomains) {
      ...
      const dumpIp = dumpedSubdomainMeta[domain]?.ip
        ? [dumpedSubdomainMeta[domain]?.ip as string]
        : [];
    ...
    const filteredEdges = edges
      .filter((edge) => allowedIpSet.has(edge.ip))
      .slice(0, 180);
    const domains = [
      ...new Set(filteredEdges.map((entry) => entry.domain)),
    ].slice(0, 30);
    const allowedDomainSet = new Set(domains);
    const finalEdges = filteredEdges.filter((edge) =>
      allowedDomainSet.has(edge.domain),
    );
    ...
    return {
      ...
      viewBoxHeight,
      truncated:
        candidateDomains.length > domains.length ||
  ...
  const rescanTargets = useMemo(() => {
    ...
    for (const subdomain of scanDiscovery.discoveredSubdomains) {
      ...
      if (classifySurfaceHostForScope(subdomain, scopeDomains).blocked)
        continue;
    ...
    scanJobs,
    scanDiscovery.discoveredSubdomains,
    scanDiscovery.discoveredIps,
    scopeDomains,
    ipScopeRules,
  ]);

  const monitoredLiveIps = useMemo(() => {
    let ips = [...scanDiscovery.discoveredIps].filter((ip) =>
      isIpWithinScopeRules(ip, ipScopeRules),
    );
  ...
  const totalAssetPages = Math.max(
    1,
    Math.ceil(monitoredLiveIps.length / assetsPerPage),
  );
  const paginatedLiveIps = monitoredLiveIps.slice(
  ...
  return (
    <DashboardLayout>
      <SurfaceScanErrorBoundary
        title="SurfaceScan360 non disponibile"
        message="I dati di scansione non sono al momento accessibili. Riprova più tardi."
      >
        <div className="space-y-6">
            ...
            <Card className="border-primary/30 bg-primary/5">
              <CardHeader>
                ...
                <p className="text-sm text-muted-foreground">
                  Aggiungi IP singoli, range o reti CIDR per controllare quali
                  asset pubblici rientrano nel monitoraggio.
                </p>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="rounded-md border border-border/70 bg-muted/20 px-3 py-2 text-xs text-muted-foreground space-y-1">
                  ...
                  <div>
                    Esempio: `dominio.it, 203.0.113.10,
                    203.0.113.10-203.0.113.20, 203.0.113.0/24`
                  </div>
                  <div>
                ...
                <div className="flex flex-col md:flex-row gap-2">
                  <Input
                    ...
                    onChange={(event) =>
                      setNewMonitoredIpInput(event.target.value)
                    }
                  ...
                  <Button
                    ...
                    disabled={
                      monitoredIpRulesSaving || !newMonitoredIpInput.trim()
                    }
                  >
                ...
                <div className="rounded-lg border border-border">
                  ...
                  {monitoredIpRulesLoading ? (
                    ...
                    <div className="p-4 text-sm text-muted-foreground">
                      Nessuna regola configurata: vengono mostrati tutti gli IP
                      scoperti.
                    </div>
                  ) : (
          ...
          <Card className="border-border">
            <CardHeader>
              <div className="flex items-start justify-between gap-3">
                <div>
                  <CardTitle>
                    Domini/IP Scansionati e Subdomain Trovati
                  </CardTitle>
                  <p className="text-sm text-muted-foreground">
                    Vista rapida dei target lanciati e degli asset scoperti via
                    enrichment OSINT.
                  </p>
                </div>
                <div className="flex items-center gap-2">
            ...
            {!isDiscoveryCollapsed ? (
              <CardContent className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  <div className="rounded-lg border border-border p-3">
                    ...
                    <div className="text-xl font-semibold">
                      {discoveredAssetsLoading
                        ? "..."
                        : scanDiscovery.discoveredSubdomains.length}
                    </div>
                  </div>
                  <div className="rounded-lg border border-border p-3">
                    <div className="text-xs text-muted-foreground">
                    ...
                    <div className="text-xl font-semibold">
                      {discoveredAssetsLoading
                        ? "..."
                        : scanDiscovery.discoveredIps.length}
                    </div>
                  </div>
                  <div className="rounded-lg border border-border p-3">
                    <div className="text-xs text-muted-foreground">
                    ...
                    <div className="text-xl font-semibold">
                      {discoveredAssetsLoading ? "..." : scopeCounters.in_scope}
                    </div>
                  </div>
                  <div className="rounded-lg border border-border p-3">
                    <div className="text-xs text-muted-foreground">
                    ...
                    <div className="text-xl font-semibold">
                      {discoveredAssetsLoading
                        ? "..."
                        : scopeCounters.excluded_by_scope}
                    </div>
                  </div>
                  <div className="rounded-lg border border-border p-3">
                    <div className="text-xs text-muted-foreground">
                      Esclusi shared/noise
                    </div>
                    <div className="text-xl font-semibold">
                      {discoveredAssetsLoading
                        ? "..."
                        : scopeCounters.excluded_shared_noise}
                    </div>
                  </div>
                </div>
                    ...
                    <div className="flex items-center justify-between gap-3">
                      <div>
                        <p className="text-sm font-medium">
                          Diagnostica Scope Guard (Admin)
                        </p>
                        <p className="text-xs text-muted-foreground">
                          Vista opzionale di elementi esclusi automaticamente da
                          scope guard.
                        </p>
                      </div>
                      <div className="flex items-center gap-2">
                      ...
                      <div className="space-y-3">
                        <div className="flex flex-wrap gap-2 text-xs">
                          <Badge variant="secondary">
                            Target esclusi: {excludedScannedTargets.length}
                          </Badge>
                          <Badge variant="secondary">
                            Host esclusi: {excludedHostDiagnostics.length}
                          </Badge>
                          <Badge variant="secondary">
                            IP esclusi scope: {scopeCounters.excluded_by_scope}
                          </Badge>
                          <Badge variant="secondary">
                            Shared/noise esclusi:{" "}
                            {scopeCounters.excluded_shared_noise}
                          </Badge>
                        </div>
                    ...
                    <TableBody>
                      {scanDiscovery.discoveredSubdomains
                        ...
                        .map((subdomain) => {
                          ...
                          const rootJob = latestJobByHost.get(
                            simpleRootDomain(subdomain),
                          );
                          ...
                          const context =
                            directJob?.hosting_context ??
                            rootJob?.hosting_context ??
                            (classification.blocked ? "excluded_noise" : null);
                          const isShared =
                            context === "shared_hosting" ||
                            context === "excluded_noise";
                          const dumpMeta = dumpedSubdomainMeta[subdomain];
                          const meta = hostMeta[subdomain];
                          const ipCandidates = [
                            ...(resolvedIpsByHost.get(subdomain) || []),
                            ...(resolvedIpsByHost.get(
                              simpleRootDomain(subdomain),
                            ) || []),
                            ...(meta?.ips || []),
                          ...
                          return (
                            <TableRow key={subdomain}>
                              ...
                              <TableCell>
                                <Badge
                                  variant={isShared ? "destructive" : "outline"}
                                >
                ...
                >
                  <div className="flex items-center justify-between gap-3 flex-wrap">
                    <div>
                      <p className="text-sm font-medium">
                        Mappa Dipendenze Dominio/IP (Scope)
                      </p>
                      <p className="text-xs text-muted-foreground">
                        Relazioni DNS operative tra domini/subdomini in scope e
                        IP associati.
                      </p>
                    </div>
                    <Badge variant="secondary">
                    ...
                    <div className="rounded-md border border-border bg-muted/10 p-3 overflow-auto">
                      ...
                      >
                        ...
                        {domainIpDependencyGraph.edges.map((edge) => {
                          const domainIndex =
                            domainIpDependencyGraph.domains.indexOf(
                              edge.domain,
                            );
                          const ipIndex = domainIpDependencyGraph.ips.indexOf(
                            edge.ip,
                          );
                        ...
                        {domainIpDependencyGraph.domains.map(
                          (domain, index) => {
                            const y = 42 + index * 28;
                            const label =
                              domain.length > 44
                  ...
                  {domainIpDependencyGraph.truncated && (
                    <p className="text-xs text-muted-foreground">
                      Mappa ottimizzata: alcune relazioni aggiuntive sono
                      disponibili nei dettagli tabellari.
                    </p>
                  )}
                </div>
                        ...
                        <TableRow key={row.host}>
                          ...
                          <TableCell className="text-sm">
                            {row.resolvedIps.length > 0
                              ? row.resolvedIps.join(", ")
                              : "-"}
                          </TableCell>
                          <TableCell className="text-sm">
                            {row.reverseHosts.length > 0
                              ? row.reverseHosts.slice(0, 4).join(", ")
                              : "-"}
                          </TableCell>
                          <TableCell>
                            <Badge
                              variant={
                                row.hostingContext === "shared_hosting"
                                  ? "destructive"
                                  : "outline"
                              }
                            >
                          ...
                          <TableCell>
                            <Badge
                              variant={row.inScope ? "default" : "secondary"}
                            >
              ...
              <CardContent className="pt-0">
                <div className="rounded-lg border border-border p-3 flex items-center justify-between gap-3 flex-wrap">
                  <div>
                    <p className="text-sm font-medium">
                      Sezione Discovery collassata
                    </p>
                    <p className="text-xs text-muted-foreground">
                      La mappa dipendenze dominio↔IP è disponibile (
                      {domainIpDependencyGraph.totalRelations} relazioni).
                    </p>
                  </div>
                  <Button
                    variant="secondary"
          ...
          <Card className="border-border">
            <CardHeader>
              <CardTitle>
                Asset IP Pubblici Monitorati ({monitoredLiveIps.length} trovati)
              </CardTitle>
              <p className="text-sm text-muted-foreground">
                    ...
                    <TableBody>
                      {paginatedLiveIps.map((ip) => {
                        const matchedRules = monitoredIpRules.filter(
                          (rule) =>
                            ["single", "range", "cidr"].includes(
                              String(rule.entry_type || "").toLowerCase(),
                            ...
                            <TableCell>
                              {matchedRules.length === 0 ? (
                                <span className="text-muted-foreground text-sm">
                                  Nessuna (visualizzazione completa)
                                </span>
                              ) : (
                                <div className="flex flex-wrap gap-2">
                  ...
                  <Pagination>
                    <PaginationContent>
                      <PaginationItem>
                        <PaginationPrevious
                          onClick={() =>
                            setAssetPage((prev) => Math.max(prev - 1, 1))
                          }
                          className={
                            assetPage === 1
                      ...
                      <PaginationItem>
                        <PaginationNext
                          onClick={() =>
                            setAssetPage((prev) =>
                              Math.min(prev + 1, totalAssetPages),
                            )
                          }
                          className={
                            ...
                            {(() => {
                              ...
                              return (
                                <div className="w-32" title={progress.title}>
                                  <div
                                    className={`h-2 rounded-full overflow-hidden ${progress.trackClass}`}
                                  >
                                    <div
