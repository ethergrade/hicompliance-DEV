...
const SurfaceScan360: React.FC = () => {
	...
	const { ipScopeRules } = useMemo(
		() =>
			splitMonitoredScopeRules(
				monitoredIpRules as Parameters<typeof splitMonitoredScopeRules>[0],
			),
		[monitoredIpRules],
	);
	...
	const scanDiscovery = useMemo(() => {
		...
		const dumpedSubdomains = (subdomainDump.history || []).flatMap((dump) =>
			(dump.results || [])
				.map((entry) =>
		...
		<DashboardLayout>
			...
			>
				<div className="space-y-6">
					...
					<SubdomainDumpPanel isAdmin={isAdminUser} dump={subdomainDump} />
