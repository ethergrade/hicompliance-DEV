...
const AdminCompanies: React.FC = () => {
	...
	const handleDeleteGroup = async (group: Group) => {
		if (
			!confirm(
				`Eliminare il gruppo "${group.name}"? Le aziende al suo interno NON saranno eliminate.`,
			)
		)
			return;
		try {
                      ...
                      `}
										>
											<div className="flex items-center gap-2 min-w-0">
												<ChevronRight
													className={`w-4 h-4 shrink-0 text-muted-foreground transition-transform ${selectedGroup?.id === g.id ? "rotate-90" : ""}`}
												/>
					...
					{!selectedGroup ? (
						<Card className="flex-1 flex items-center justify-center">
							<CardContent className="py-16 text-center text-muted-foreground">
								...
								<p>
									Seleziona un'azienda dalla sidebar per visualizzare i suoi
									clienti.
								</p>
							</CardContent>
						</Card>
							...
							<CardHeader className="shrink-0 space-y-3">
								<div className="flex items-center justify-between">
									<CardTitle className="flex items-center gap-2">
										...
										Clienti ({filteredTenants.length}
															...
															<td className="py-3 px-3">
																<div className="flex items-center justify-end gap-1">
																	...
																	<Button
																		...
																		size="icon"
																		onClick={() =>
																			setDeletingOrg({ id: t.id, name: t.name })
																		}
																		title="Elimina"
																	>
