...
const AdminReporting: React.FC = () => {
	...
	const fetchAggregatedData = async () => {
		...
		try {
			...
			const orgStats: OrganizationStats[] = (orgs || []).map((org) => {
				const orgUsers =
					users?.filter((u) => u.organization_id === org.id) || [];
				const orgServices =
					services?.filter((s) => s.organization_id === org.id) || [];
				const orgTasks =
					tasks?.filter((t) => t.organization_id === org.id && !t.is_deleted) ||
					[];
				const orgResponses =
					responses?.filter((r) => r.organization_id === org.id) || [];

				const activeServices = orgServices.filter(
					(s) => s.status === "active",
				).length;
				const completedResponses = orgResponses.filter(
				...
				const completedTasks = orgTasks.filter(
					(t) => t.progress === 100,
				).length;
			...
			const totalActiveServices = orgStats.reduce(
				(acc, org) => acc + org.activeServicesCount,
				0,
			);
			const avgProgress =
				totalOrgs > 0
					? Math.round(
							orgStats.reduce((acc, org) => acc + org.assessmentProgress, 0) /
								totalOrgs,
						)
					: 0;
			const totalTasks = orgStats.reduce(
				(acc, org) => acc + org.remediationTasksCount,
				0,
			);
			const completedTasks = orgStats.reduce(
				(acc, org) => acc + org.completedTasksCount,
				0,
			);
			const avgRisk =
				totalOrgs > 0
					? Math.round(
							orgStats.reduce((acc, org) => acc + org.riskScore, 0) / totalOrgs,
						)
					: 0;
	...
	const riskTrendData = [
		...
		{
			month: "Feb",
			avgRisk: aggregatedStats.averageRiskScore || 58,
			incidents: 18,
		},
	];
						...
						<CardContent className="p-4">
							<div className="flex items-center justify-between">
								<div>
									...
									<p className="text-3xl font-bold text-foreground">
										{aggregatedStats.totalActiveServices}
									</p>
						...
						<CardContent className="p-4">
							<div className="flex items-center justify-between">
								<div>
									...
									<p
										className={`text-3xl font-bold ${getRiskColor(aggregatedStats.averageRiskScore)}`}
									>
							...
							<div className="flex items-center justify-between mb-3">
								<div>
									...
									<p className="text-2xl font-bold text-foreground">
										{aggregatedStats.averageAssessmentProgress}%
									</p>
								</div>
								<Target className="w-6 h-6 text-primary" />
							</div>
							<Progress
								value={aggregatedStats.averageAssessmentProgress}
								className="h-2"
							/>
						...
						<CardContent className="p-4">
							<div className="flex items-center justify-between mb-3">
								<div>
									...
									<p className="text-2xl font-bold text-foreground">
										{aggregatedStats.completedRemediationTasks}/
										{aggregatedStats.totalRemediationTasks}
									</p>
								</div>
										...
										>
											...
											<td className="text-center py-3 px-4">
												<span className="text-foreground">
													{org.activeServicesCount}/{org.servicesCount}
												</span>
											</td>
											<td className="text-center py-3 px-4">
												<div className="flex items-center justify-center gap-2">
													<Progress
														value={org.assessmentProgress}
														className="w-16 h-2"
													/>
											...
											<td className="text-center py-3 px-4">
												<span className="text-foreground">
													{org.completedTasksCount}/{org.remediationTasksCount}
												</span>
											</td>
											<td className="text-center py-3 px-4">
