	...
	const handleExternalClick = async (
		...
		try {
			...
			toast({
				title: "Link copiato",
				description:
					"In preview i popup possono essere bloccati. Incolla il link nel browser per aprirlo.",
			});
		} catch {
			toast({
			...
			<div className="flex items-center justify-between gap-2">
				<div className="flex items-center gap-2">
					...
					{item.type === "cve" && item.epssScore != null && (
						<div
							className={`flex items-center gap-1 text-xs font-medium ${getEpssColor(item.epssScore)}`}
						>
