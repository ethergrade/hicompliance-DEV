...
// SurfaceScan module data is read through the hiconsole backend API.
import {
	surfaceScan360Api,
	type SurfaceScanJob,
...
const headerRules: Array<{ key: string; label: string; remediation: string }> =
	[
		...
		},
		{
			...
			remediation:
				"Impostare X-Content-Type-Options: nosniff per limitare MIME sniffing.",
		},
		{
			key: "xFrameOptions",
			label: "X-Frame-Options / frame-ancestors",
			remediation:
				"Impostare X-Frame-Options o CSP frame-ancestors per prevenire clickjacking.",
		},
		{
			key: "referrerPolicy",
			label: "Referrer-Policy",
			remediation:
				"Impostare una Referrer-Policy restrittiva (es. strict-origin-when-cross-origin).",
		},
		{
			key: "permissionsPolicy",
			label: "Permissions-Policy",
			remediation:
				"Definire una Permissions-Policy minima per ridurre superfici browser inutili.",
		},
		{
		...
		},
		{
			...
			label: "CORP",
			remediation:
				"Valutare CORP per controllare uso cross-origin delle risorse.",
		},
		{
			key: "crossOriginEmbedderPolicy",
			label: "COEP",
			remediation:
				"Valutare COEP in combinazione con COOP/CORP dove applicabile.",
		},
	];

const SENSITIVE_SUBDOMAIN_REGEX =
	/(staging|dev|test|backup|vpn|cpanel|webmail|autodiscover|mail)/i;
...
const formatModuleDiagnostic = (
	...
	const moduleLabel =
		moduleKey === "http_security" ? "HTTP Security" : "Modulo";
	...
	if (
		/no route to host|network is unreachable|host unreachable/i.test(lowered)
	) {
	...
	if (
		/connection refused|connect error|connection reset|connection closed/i.test(
			lowered,
		)
	) {
...
const outcomeFromJobStatus = (status: string): ModuleOutcomeStatus => {
	...
	if (key === "completed" || key === "success" || key === "partial")
...
const FAILED_LIVE_STATUSES = new Set([
	"failed",
	"error",
	"stopped",
	"aborted",
...
const scopeLiveReasonLabel = (row: ScopeTargetRow): string => {
	...
	if (FAILED_LIVE_STATUSES.has(live))
		return row.snapshotSource === "last_good" ? "last good snapshot" : "failed";
...
const moduleReasonLabel = (reason: string): string => {
	...
	if (key.includes("missing_google_cloud_api_key"))
		return "Prerequisito mancante: GOOGLE_API_KEY per Quality.";
	if (key.includes("feature_flag_disabled"))
		return "Modulo disabilitato da feature flag.";
	if (key.includes("rdap"))
		return "RDAP temporaneamente non disponibile per WHOIS.";
	if (key.includes("server_location_no_ip"))
		return "Nessun IP in-scope geolocalizzabile disponibile.";
	if (key.includes("open_ports_no_data"))
		return "Nessun dato porte disponibile da scan classica/exposure.";
...
const severityForPort = (
	port: number,
): "critical" | "high" | "medium" | "low" | "info" => {
	if ([3389, 5900, 6379, 9200, 9300, 27017, 11211].includes(port))
		return "critical";
	if ([21, 23, 445, 3306, 5432, 1521, 5060].includes(port)) return "high";
	if ([22, 25, 8080, 8443, 9443, 8000, 9000, 9090, 8081].includes(port))
		return "medium";
...
export const SurfaceScanModuleCards: React.FC<SurfaceScanModuleCardsProps> = ({
	...
	const [latestLiveScopeJobs, setLatestLiveScopeJobs] = useState<
		Record<string, LatestScanRow>
	>({});
	const [configuredScopeTargets, setConfiguredScopeTargets] = useState<
	...
	const [exposureOpenPorts, setExposureOpenPorts] = useState<
		ExposureOpenPortRow[]
	>([]);
	...
	useEffect(() => {
		...
		const fetchData = async () => {
			...
			try {
				...
				const scopeRules = (await surfaceScan360Api.listMonitoredIps(
					organizationId,
					groupId,
				)) as SurfaceMonitoredScopeRule[];

				// Fetch jobs from backend API
				const apiJobs = await surfaceScan360Api.listJobs(
					organizationId,
					{ status: "all" },
					groupId,
				);
				const { scopeDomains, ipScopeRules } =
				...
				for (const job of jobs) {
					const targetRaw = String(
						job.normalized_target || job.raw_target || "",
					).trim();
					...
					if (host) {
						if (isIpv4(host) || isIpv6(host)) {
							...
							const classification = classifySurfaceHostForScope(
								host,
								scopeDomains,
							);
				...
				for (const [targetKey, targetJobs] of groupedByTarget.entries()) {
					...
					const completedWithScore = sorted.find((entry) => {
						...
						if (!["completed", "partial", "success"].includes(status))
							return false;
						const score = Number(entry.summary?.overall_score);
						return Number.isFinite(score);
					});
					const completedAny = sorted.find((entry) =>
						["completed", "partial", "success"].includes(
							String(entry.status || "").toLowerCase(),
						),
					);
					selectedByTarget.set(
						targetKey,
						completedWithScore || completedAny || liveJob,
					);
				}
				...
				const latestLive = Object.values(liveByTarget).sort(
					(a, b) => Date.parse(b.created_at) - Date.parse(a.created_at),
				)[0];
				...
				if (completedJobIds.length > 0) {
					const findingsResults = await Promise.allSettled(
						completedJobIds.map((jobId) =>
							surfaceScan360Api.getJobFindings(
								organizationId,
								jobId,
								{},
								groupId,
				...
				// Module results, observations, open ports — backend API, chunked by job IDs.
				const [moduleChunks, observationChunks, exposureChunks] =
					await Promise.all([
						Promise.all(
							chunk(jobIds, 40).map((group) =>
								surfaceScan360Api.getModuleResults(
									organizationId,
									{ job_ids: group.join(",") },
									groupId,
								),
						...
						Promise.all(
							chunk(jobIds, 40).map((group) =>
								surfaceScan360Api.getObservations(
									organizationId,
									{ job_ids: group.join(",") },
									groupId,
								),
						...
						Promise.all(
							chunk(jobIds, 40).map((group) =>
								surfaceScan360Api.getOpenPorts(
									organizationId,
									{ job_ids: group.join(",") },
									groupId,
								),
				...
				const moduleRows = moduleChunks.flat() as ModuleResultRow[];
				const observationRows = observationChunks.flat() as ObservationRow[];
				const exposureRows = exposureChunks.flat() as ExposureOpenPortRow[];
	...
	const scopeTargetRows = useMemo<ScopeTargetRow[]>(() => {
		const scannedRows = latestScopeJobs.map((job) => {
			const rawLabel = String(
				job.raw_target || job.normalized_target || "-",
			).trim();
			...
			const riskFromSummary = String(
				job.summary?.risk_level || "",
			).toLowerCase();
	...
	useEffect(() => {
		...
		setSelectedScopeTargetKey((prev) => {
			if (prev && scopeTargetRows.some((row) => row.targetKey === prev))
				return prev;
			const preferred =
				scopeTargetRows.find((row) => row.jobId) || scopeTargetRows[0];
	...
	const selectedScopeTargetRow = useMemo<ScopeTargetRow | null>(() => {
		...
		return (
			scopeTargetRows.find((row) => row.targetKey === selectedScopeTargetKey) ||
			scopeTargetRows[0]
		);
		...
		() =>
			selectedScopeJobId
				? moduleResults.filter(
						(row) => String(row.scan_job_id) === selectedScopeJobId,
					)
				: [],
	...
	const qualityRows = useMemo(
		() =>
			selectedObservations.filter(
				(row) =>
					row.module === "quality" &&
					row.observation_type.startsWith("quality_summary"),
			),
		[selectedObservations],
	);
	...
	const qualityCategories = useMemo(() => {
		const samples = qualityRows
			...
			.filter((entry) => entry && typeof entry === "object") as Array<
		...
		const avg = (key: string) => {
			...
			return Math.round(
				vals.reduce((acc, value) => acc + value, 0) / vals.length,
			);
		};
	...
	const qualityFailedAudits = useMemo(() => {
		...
		for (const row of qualityRows) {
			const failed = Array.isArray(row.value?.failed_audits)
				? row.value.failed_audits
				: [];
	...
	const latestWhois = useMemo(() => {
		const whoisRows = selectedObservations
			.filter(
				(row) =>
					row.module === "whois" && row.observation_type === "whois_rdap",
			)
	...
	const whoisUnavailableCount = useMemo(
		() =>
			selectedObservations.filter(
				(row) =>
					row.module === "whois" && row.observation_type === "rdap_unavailable",
			).length,
		[selectedObservations],
	);
	...
	const latestServerLocation = useMemo(() => {
		const rows = selectedObservations
			.filter(
				(row) =>
					row.module === "server_location" &&
					row.observation_type === "server_location",
			)
			.sort((a, b) => Date.parse(b.created_at) - Date.parse(a.created_at));
		return rows[0]?.value || {};
	}, [selectedObservations]);
	...
	const openPortRows = useMemo<OpenPortRow[]>(() => {
		...
		const observedRows = selectedObservations
			.filter(
				(row) =>
					row.module === "open_ports" &&
					row.observation_type === "open_ports_summary",
			)
			.flatMap((row) =>
				Array.isArray(row.value?.openPorts) ? row.value.openPorts : [],
			);
		...
		return Array.from(merged.values()).sort((a, b) => {
			const sevDelta =
				severityRank[severityForPort(b.port)] -
				severityRank[severityForPort(a.port)];
	...
	const moduleOutcomes = useMemo(() => {
		...
		for (const moduleKey of moduleOrder) {
			...
			const hasError = rows.some((entry) =>
				["error", "timeout"].includes(entry.status),
			);
			...
			const hasData = (() => {
				...
				if (moduleKey === "quality") {
					return (
						Number(qualityCategories.performance || 0) > 0 ||
				...
				if (moduleKey === "server_location") {
					return Boolean(
						latestServerLocation?.city ||
							latestServerLocation?.country ||
							latestServerLocation?.ip,
					);
				}
				return obs.some((entry) => {
					if (
						[
							...
							"module_timeout",
							"rdap_unavailable",
						].includes(entry.observation_type)
					) {
						return false;
					}
			...
			const hasPrereqSkip = obs.some((entry) => {
				...
				return (
					reason.includes("missing_google_cloud_api_key") ||
					reason.includes("feature_flag_disabled")
				);
			});
			const hasRdapUnavailable =
				moduleKey === "whois" &&
				obs.some((entry) => entry.observation_type === "rdap_unavailable");
			const hasServerLocationNoIp =
				moduleKey === "server_location" &&
				!hasData &&
				hasSuccess &&
				obs.length === 0;
			const hasOpenPortsNoData =
				moduleKey === "open_ports" && !hasData && hasSuccess;
	...
	const moduleSkipReasons = useMemo(() => {
		...
		for (const row of selectedObservations) {
			...
			if (
				row.module === "whois" &&
				row.observation_type === "rdap_unavailable" &&
				!out[row.module]
			) {
	...
	const moduleDiagnostics = useMemo(() => {
		...
		for (const row of selectedObservations) {
			if (!["module_error", "module_timeout"].includes(row.observation_type))
				continue;
			const message = firstText(
				row.value?.error,
				row.value?.message,
	...
	const filteredScopeTargetRows = useMemo(() => {
		...
		return scopeTargetRows.filter((entry) => {
			const text =
				`${entry.label} ${entry.profile} ${entry.status}`.toLowerCase();
	...
	const httpSecurityEvaluated =
		httpSecurityOutcome === "success_with_data" ||
	...
	const subdomainTotalPages = Math.max(
		1,
		Math.ceil(filteredSubdomains.length / subdomainsPerPage),
	);
			...
			<Card className="border-border">
				...
				<CardContent>
					<p className="text-sm text-muted-foreground">
						Nessuna regola in scope o scansione disponibile.
					</p>
				</CardContent>
			</Card>
		);
		...
		<TooltipProvider>
			<Card className="border-border">
				<CardHeader className="pb-3">
					...
					<p className="text-sm text-muted-foreground">
						Vista unificata dei target in scope con stato automatico scansioni,
				...
				<CardContent className="space-y-5">
					<div className="grid grid-cols-1 md:grid-cols-3 xl:grid-cols-6 gap-3">
						...
						<div className="rounded-lg border border-border p-3">
							...
							<p className="text-sm font-medium">
								{scopeStatusCounters.notScanned}
							</p>
						</div>
						<div className="rounded-lg border border-border p-3">
							<p className="text-xs text-muted-foreground">Score medio scope</p>
							<Badge className={scoreTone(scoreSummary.overallScore)}>
								{scoreSummary.overallScore}/100
							</Badge>
						</div>
						<div className="rounded-lg border border-border p-3">
							<p className="text-xs text-muted-foreground">
								Ultimo aggiornamento scope
							</p>
							<p className="text-xs text-foreground">
								{latestScan?.completed_at
									? new Date(latestScan.completed_at).toLocaleString("it-IT")
									: "-"}
							</p>
						</div>
					</div>
					...
					<div className="rounded-xl border border-border/80 bg-card/40 p-4 space-y-3">
						<div className="flex flex-wrap items-start justify-between gap-3">
							<div>
								<p className="text-sm font-semibold text-foreground">
									Panoramica Completa Target In Scope
								</p>
								<p className="text-xs text-muted-foreground">
									Tutti i domini/IP in-scope con ultimo stato scansione, profilo
									e rischio.
								</p>
							</div>
							<Badge variant="secondary">
								{filteredScopeTargetRows.length} target visibili
							</Badge>
						</div>
						...
						<div className="rounded-lg border border-primary/40 bg-primary/5 p-3">
							<div className="flex items-center justify-between gap-3 flex-wrap">
								<div>
									<p className="text-xs text-muted-foreground">
										Target attivo per i controlli modulo
									</p>
									<p className="text-sm font-semibold text-foreground break-all">
										{selectedScopeTargetRow?.label || "-"}
									</p>
								</div>
								<div className="flex items-center gap-2 text-xs">
									<Badge variant="outline">
										{selectedScopeTargetRow?.profile || "n/d"}
									</Badge>
									{selectedScopeTargetRow?.status === "not_scanned" ? (
										...
										<Badge
											className={
												statusBadgeClass[
													outcomeFromJobStatus(
														selectedScopeTargetRow
															? effectiveScopeStatus(selectedScopeTargetRow)
															: "not_scanned",
													)
												]
											}
									...
									{selectedScopeTargetRow?.snapshotSource === "last_good" && (
										<Badge variant="secondary">
											Dati da ultimo snapshot valido
										</Badge>
									)}
								</div>
							</div>
								...
								return (
									...
									>
										<div className="flex items-start justify-between gap-2">
											...
											{entry.status === "not_scanned" ? (
												...
												<Badge
													className={
														statusBadgeClass[
															outcomeFromJobStatus(effectiveScopeStatus(entry))
														]
													}
												>
										...
										<div className="flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
											...
											{entry.score != null ? (
												<Badge className={scoreTone(entry.score)}>
													{entry.score}/100
												</Badge>
											) : (
										...
										<p className="text-[11px] text-muted-foreground">
											Live:{" "}
											{entry.liveCreatedAt
												? new Date(entry.liveCreatedAt).toLocaleString("it-IT")
												: "-"}
					...
					{selectedRiskFindings.length > 0 && (
						<div className="rounded-lg border border-border p-3 space-y-2">
							<div className="flex items-center gap-2">
								...
								<p className="text-sm font-medium">
									Risk findings prioritari (critical/high) · target attivo
								</p>
							</div>
										...
										>
											<div className="flex items-center gap-2 flex-wrap">
												...
												<span className="font-medium">
													{finding.title || finding.finding_type || "Finding"}
												</span>
											</div>
											{targets.length > 0 && (
												<div className="flex flex-wrap gap-1.5 mt-1.5">
													{targets.map((target) => (
														<Badge
															key={`${finding.finding_type}-${index}-${target}`}
															variant="outline"
															className="text-[10px]"
														>
															{target}
					...
					<div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">
						<div className="rounded-lg border border-border p-3 space-y-3">
							<div className="flex items-center justify-between">
								...
								<Badge
									className={
										statusBadgeClass[moduleOutcomes.passes || "success_no_data"]
									}
								>
							...
							<div className="text-xs text-muted-foreground">
								Score: {toPercent(httpSecurity.score)} / 100 · HTTP{" "}
								{httpSecurity.statusCode ?? "-"}
							</div>
							...
							{!httpSecurityDiagnostic && !httpSecurityEvaluated && (
								<div className="rounded-md border border-amber-500/25 bg-amber-500/10 p-2 text-xs text-amber-100">
									Controllo HTTP Security non ancora valutato per questo target:
									lo stato verra aggiornato dal prossimo ciclo di scansione.
								</div>
							)}
						...
						<div className="rounded-lg border border-border p-3 space-y-3">
							<div className="flex items-center justify-between">
								...
								<Badge
									className={
										statusBadgeClass[moduleOutcomes.dnssec || "success_no_data"]
									}
								>
							...
							<div className="text-xs space-y-1">
								<div className="flex justify-between">
									<span>DNSKEY</span>
									<span>{dnssec.dnskey_present ? "Presente" : "Assente"}</span>
								</div>
								<div className="flex justify-between">
									<span>DS</span>
									<span>{dnssec.ds_present ? "Presente" : "Assente"}</span>
								</div>
								<div className="flex justify-between">
									<span>RRSIG</span>
									<span>{dnssec.rrsig_present ? "Presente" : "Assente"}</span>
								</div>
							</div>
							<p className="text-xs text-muted-foreground">
								Abilitare DNSSEC presso registrar/provider DNS e verificare DS
								in parent zone.
							</p>
						</div>
						...
						<div className="rounded-lg border border-border p-3 space-y-3">
							<div className="flex items-center justify-between">
								...
								<Badge
									className={
										statusBadgeClass[
											moduleOutcomes.quality || "success_no_data"
										]
									}
								>
							...
							<div className="text-xs text-muted-foreground">
								Coverage: {qualityRows.length} target
							</div>
							<div className="space-y-2 text-xs">
								{[
									...
									[
										"Best Practices",
										toPercent(qualityCategories.best_practices),
									],
							...
							{moduleOutcomes.quality === "skipped_prerequisite" && (
								<p className="text-xs text-amber-300">
									{moduleReasonLabel(moduleSkipReasons.quality || "")}
								</p>
							)}
						</div>
						...
						<div className="rounded-lg border border-border p-3 space-y-3">
							<div className="flex items-center justify-between">
								...
								<Badge
									className={
										statusBadgeClass[
											moduleOutcomes.open_ports || "success_no_data"
										]
									}
								>
							...
							<div className="space-y-1.5 max-h-40 overflow-auto pr-1">
								...
								{openPortRows.length === 0 && (
									<p className="text-xs text-muted-foreground">
										Nessuna porta aperta disponibile.
									</p>
								)}
							</div>
							{moduleOutcomes.open_ports === "skipped_prerequisite" && (
								<p className="text-xs text-amber-300">
									{moduleReasonLabel(moduleSkipReasons.open_ports || "")}
								</p>
							)}
						</div>
						...
						<div className="rounded-lg border border-border p-3 space-y-3">
							<div className="flex items-center justify-between">
								...
								<Badge
									className={
										statusBadgeClass[
											moduleOutcomes.threats || "success_no_data"
										]
									}
								>
							...
							<div className="space-y-1.5 text-xs">
								<div className="flex justify-between">
									...
									<Badge variant="outline">
										{threats?.safe_browsing?.unsafe ? "Unsafe" : "Safe"}
									</Badge>
								</div>
								<div className="flex justify-between">
									<span>URLHaus</span>
									<Badge variant="outline">
										{threats?.urlhaus?.listed ? "Listed" : "Not listed"}
									</Badge>
								</div>
								<div className="flex justify-between">
									<span>PhishTank</span>
									<Badge variant="outline">
										{threats?.phishtank?.verified
											? "Phishing found"
											: "No phishing"}
									</Badge>
								</div>
								<div className="flex justify-between">
									<span>IOC Fresh List</span>
									<Badge variant="outline">
										{iocFreshList?.matched
											? `Match (${Number(iocFreshList?.matched_count || 0)})`
											: "No match"}
									</Badge>
								</div>
								<div className="flex justify-between">
									<span>Stato feed</span>
									<Badge className={iocFreshBadge.className}>
										{iocFreshBadge.label}
									</Badge>
								</div>
								<div className="flex justify-between">
									<span>Lease</span>
									<Badge variant="outline">
										{iocLeaseMinutes ? `${iocLeaseMinutes} min` : "-"}
									</Badge>
								</div>
								<div className="flex justify-between">
									<span>Ultimo refresh</span>
									<Badge variant="outline">{iocLastRefreshedLabel}</Badge>
								</div>
								<div className="flex justify-between">
									<span>DNS Blocklist</span>
									<Badge variant="outline">
										{Number(blocklists?.listed_count || 0) > 0
											? `Listed (${blocklists?.listed_count})`
											: "Clean"}
									</Badge>
								</div>
							</div>
						...
						<div className="rounded-lg border border-border p-3 space-y-3">
							<div className="flex items-center justify-between">
								...
								<Badge
									className={
										statusBadgeClass[moduleOutcomes.whois || "success_no_data"]
									}
								>
							...
							<div className="text-xs space-y-1">
								<div className="flex justify-between">
									<span>Coverage</span>
									<span>{whoisCoverage} target</span>
								</div>
								<div className="flex justify-between">
									<span>RDAP unavailable</span>
									<span>{whoisUnavailableCount}</span>
								</div>
								<div className="flex justify-between">
									<span>Registrar</span>
									<span>{latestWhois.registrar || "-"}</span>
								</div>
								<div className="flex justify-between">
									<span>Scadenza</span>
									<span>
										{latestWhois.days_to_expiry != null
											? `${latestWhois.days_to_expiry} giorni`
											: "-"}
									</span>
								</div>
								<div className="flex justify-between">
									<span>DNSSEC (RDAP)</span>
									<span>{latestWhois.dnssec || "-"}</span>
								</div>
							</div>
							{whoisUnavailableCount > 0 &&
								moduleOutcomes.whois !== "skipped_prerequisite" && (
									<p className="text-xs text-amber-300">
										RDAP temporaneamente non disponibile per WHOIS.
									</p>
								)}
							{moduleOutcomes.whois === "skipped_prerequisite" && (
								<p className="text-xs text-amber-300">
									{moduleReasonLabel(moduleSkipReasons.whois || "")}
								</p>
							)}
						</div>
						...
						<div className="rounded-lg border border-border p-3 space-y-3">
							<div className="flex items-center justify-between">
								...
								<Badge
									className={
										statusBadgeClass[
											moduleOutcomes.ssl_certificate || "success_no_data"
										]
									}
								>
									{statusLabel(
							...
							<div className="text-xs space-y-1">
								...
								<div className="flex justify-between">
									<span>Scadenza cert</span>
									<span>
										{ssl.expiresInDays != null
											? `${ssl.expiresInDays} giorni`
											: "-"}
									</span>
								</div>
								<div className="flex justify-between">
									<span>TLS 1.2+</span>
									<span>
										{tls.tls12Supported || tls.tls13Supported ? "Sì" : "No"}
									</span>
								</div>
							</div>
						</div>
						...
						<div className="rounded-lg border border-border p-3 space-y-3">
							<div className="flex items-center justify-between">
								...
								<Badge
									className={
										statusBadgeClass[
											moduleOutcomes.server_info || "success_no_data"
										]
									}
								>
							...
							<div className="text-xs text-muted-foreground">
								ASN: {serverInfo.asn || "-"} · Org:{" "}
								{serverInfo.organization || "-"}
							</div>
							<div className="text-xs text-muted-foreground">
								Porte note:{" "}
								{Array.isArray(serverInfo.ports) ? serverInfo.ports.length : 0}
							</div>
						</div>
						...
						<div className="rounded-lg border border-border p-3 space-y-3">
							<div className="flex items-center justify-between">
								...
								<Badge
									className={
										statusBadgeClass[
											moduleOutcomes.server_location || "success_no_data"
										]
									}
								>
									{statusLabel(
							...
							<div className="text-xs text-muted-foreground">
								Coverage: {serverLocationCoverage} target
							</div>
							<div className="text-xs text-muted-foreground">
								{latestServerLocation.city || "-"},{" "}
								{latestServerLocation.countryCode ||
									latestServerLocation.country ||
									"-"}
							</div>
							{serverLocationCoverage === 0 &&
								moduleOutcomes.server_location !== "skipped_prerequisite" && (
									<p className="text-xs text-amber-300">
										Nessun IP in-scope geolocalizzabile disponibile.
									</p>
								)}
							{moduleOutcomes.server_location === "skipped_prerequisite" && (
								<p className="text-xs text-amber-300">
									{moduleReasonLabel(moduleSkipReasons.server_location || "")}
								</p>
							)}
						</div>
						...
						<div className="rounded-lg border border-border p-3 space-y-3">
							<div className="flex items-center justify-between">
								...
								<Badge
									className={
										statusBadgeClass[
											moduleOutcomes.mail_config || "success_no_data"
										]
									}
								>
							...
							<div className="text-xs space-y-1">
								<div className="flex justify-between">
									<span>SPF</span>
									<span>
										{Array.isArray(mailConfig.spf_records) &&
										mailConfig.spf_records.length > 0
											? "Presente"
											: "Assente"}
									</span>
								</div>
								<div className="flex justify-between">
									<span>DMARC</span>
									<span>
										{Array.isArray(mailConfig.dmarc_records) &&
										mailConfig.dmarc_records.length > 0
											? "Presente"
											: "Assente"}
									</span>
								</div>
								<div className="flex justify-between">
									...
									<span>
										{Array.isArray(mailConfig.dkim_selectors_found)
											? mailConfig.dkim_selectors_found.length
											: 0}
									</span>
								</div>
							</div>
						...
						<div className="rounded-lg border border-border p-3 space-y-3">
							<div className="flex items-center justify-between">
								...
								<Badge
									className={
										statusBadgeClass[
											moduleOutcomes.redirects ||
												moduleOutcomes.redirect_chain ||
												"success_no_data"
										]
									}
								>
							...
							<div className="text-xs space-y-1">
								<div className="flex justify-between">
									<span>Hop count</span>
									<span>{redirects.hopCount ?? "-"}</span>
								</div>
								<div className="flex justify-between">
									<span>HTTP→HTTPS</span>
									<span>{redirects.redirectsToHttps ? "Sì" : "No"}</span>
								</div>
								<div className="flex justify-between">
									<span>External redirect</span>
									<span>{redirects.externalRedirect ? "Sì" : "No"}</span>
								</div>
							</div>
						</div>
					</div>
					...
					<div className="rounded-lg border border-border p-3 space-y-3">
						<div className="flex items-center justify-between gap-3">
							<div>
								...
								<p className="text-xs text-muted-foreground">
									Base domain scope · paginazione · filtro rapido
								</p>
							</div>
							<Button
								size="sm"
									...
									<div className="flex items-center justify-between gap-2 flex-wrap">
										...
										<div className="flex items-center gap-1">
											<Button
												...
												onClick={() => void handleAddScope(entry)}
											>
												Add to scope
											</Button>
											<Button
												...
												variant="ghost"
												className="h-7 text-xs"
												onClick={() => void handleScanSubdomain(entry)}
											>
												Scan selected
											</Button>
										</div>
						...
						{subdomainTotalPages > 1 && (
							<div className="flex items-center justify-end gap-2">
								<Button
									...
									onClick={() =>
										setSubdomainPage((prev) => Math.max(prev - 1, 1))
									}
								>
									<ChevronLeft className="w-4 h-4" />
								</Button>
								<span className="text-xs text-muted-foreground">
									{subdomainPage}/{subdomainTotalPages}
								</span>
								<Button
									size="icon"
									variant="outline"
									...
									onClick={() =>
										setSubdomainPage((prev) =>
											Math.min(prev + 1, subdomainTotalPages),
										)
									}
								>
