...
// SurfaceScan exposure data is read through the hiconsole backend API.
import {
	surfaceScan360Api,
	type SurfaceScanJob,
...
const targetMatchKey = (value: string): string => {
	...
	try {
		...
		return String(parsed.hostname || raw)
			.trim()
			.toLowerCase();
	} catch {
...
const dedupeTechnologiesRows = (
	rows: ExposureTechnologyRow[],
): ExposureTechnologyRow[] => {
...
export const SurfaceScanExposureSection: React.FC<
	SurfaceScanExposureSectionProps
> = ({ isAdmin }) => {
	...
	const [targetSnapshots, setTargetSnapshots] = useState<
		ExposureSummary["target_snapshots"]
	>([]);
	...
	const refreshData = useCallback(async () => {
		...
		setLoading(true);
		try {
			const [jobsResult, summaryResult, scopeDomainsResult] =
				await Promise.allSettled([
					surfaceScan360Api.listJobs(
						organizationId,
						{ status: "all" },
						groupId,
					),
					surfaceScan360Api.getExposureSummary(
						organizationId,
						{ scope_mode: "scope_latest_per_target" },
						groupId,
					),
			...
			const scopeDomainsData =
				scopeDomainsResult.status === "fulfilled"
					? scopeDomainsResult.value
					: null;
			const monitoredRules: Array<{ entry_type: string; input_value: string }> =
				Array.isArray(scopeDomainsData) ? scopeDomainsData : [];
			const normalizedScopeDomains = Array.from(
				new Set(
					monitoredRules
						.filter(
							(entry) =>
								String(entry.entry_type || "").toLowerCase() === "domain",
						)
			...
			const normalizedScopePublicIps = Array.from(
				new Set(
					monitoredRules
						.filter(
							(entry) =>
								String(entry.entry_type || "").toLowerCase() === "single",
						)
			...
			const effectiveJobIds = (summaryData?.job_ids || [])
			...
			const jobIdChunks = Array.from(
				{ length: Math.ceil(effectiveJobIds.length / 40) },
				(_, index) => effectiveJobIds.slice(index * 40, index * 40 + 40),
			);
			...
			const [portsRes, techRes, findingsRes2] = await Promise.allSettled([
				...
				).then((chunks) => chunks.flat() as ExposureOpenPortRow[]),
				Promise.all(
					jobIdChunks.map((ids) =>
						surfaceScan360Api.getTechnologies(
							organizationId,
							{ job_ids: ids.join(",") },
							groupId,
						),
					),
				).then((chunks) => chunks.flat() as ExposureTechnologyRow[]),
				Promise.all(
					effectiveJobIds.map((jobId) =>
						surfaceScan360Api.getExposureFindings(
							organizationId,
							jobId,
							{},
							groupId,
				...
				).then((chunks) => chunks.flat() as ExposureFindingRow[]),
			]);
			setOpenPorts(
				portsRes.status === "fulfilled"
					? dedupeOpenPortsRows(portsRes.value)
					: [],
			);
			setTechnologies(
				techRes.status === "fulfilled"
					? dedupeTechnologiesRows(techRes.value)
					: [],
			);
			setFindings(
				findingsRes2.status === "fulfilled"
					? dedupeFindingsRows(findingsRes2.value)
					: [],
			);
		} catch (error: unknown) {
			console.error("Exposure refresh error:", error);
			toast.error("Impossibile caricare dati exposure", {
				description: getErrorMessage(error, "Errore di caricamento"),
			});
		} finally {
			setLoading(false);
		}
	...
	const runExposureScan = useCallback(
		async (payload: ExposureStartRequest, options?: { auto?: boolean }) => {
			...
			try {
				const result = await startExposureScan(
					payload,
					organizationId ?? "",
					groupId,
				);
				...
				toast.success(
					options?.auto
						? "Scansione scope avviata automaticamente"
						: "Scansione exposure avviata",
					{
				...
				toast.error(
					options?.auto
						? "Auto-avvio scope non riuscito"
						: "Avvio scansione non riuscito",
					{
						description: getErrorMessage(error, "Errore durante avvio"),
					},
				);
				throw error;
			} finally {
	...
	const handleStartScan = async () => {
		...
		const normalizedDomains = Array.from(
			new Set(scopeDomains.map(normalizeScopeDomain).filter(Boolean)),
		);
		const normalizedIps = Array.from(
			new Set(scopePublicIps.map(normalizeScopeIp).filter(Boolean)),
		);
		...
		const payload: ExposureStartRequest = {
			...
			scan_name:
				scanName.trim() ||
				`Exposure Full Scan · Scope ${new Date().toISOString().slice(0, 16)}`,
	...
	useEffect(() => {
		...
		const isStale =
			!latestJobTs || Date.now() - latestJobTs > 1000 * 60 * 60 * 12;
		...
		const scannedTargetKeys = new Set(
			jobs
				.map((job) =>
					targetMatchKey(
						String(job?.normalized_target || job?.raw_target || ""),
					),
				)
				.filter(Boolean),
		);
		...
		const missingScopeTargets = Array.from(scopeTargetKeys).filter(
			(key) => !scannedTargetKeys.has(key),
		);
	...
	const handlePoll = async () => {
		...
		try {
			...
			toast.error("Poll non riuscito", {
				description: getErrorMessage(error, "Errore di polling"),
			});
		} finally {
			setPolling(false);
		}
	...
	const handleResync = async () => {
		...
		try {
			...
			toast.error("Resync non riuscito", {
				description: getErrorMessage(error, "Errore durante resync"),
			});
		} finally {
			setPolling(false);
		}
				...
				<div className="flex items-start justify-between gap-3 flex-wrap">
					<div>
						<CardTitle className="text-xl">
							Ports & Technologies (Scope Unificato)
						</CardTitle>
						<p className="text-sm text-muted-foreground">
							Dati exposure consolidati su tutti i target in scope
							(latest-per-target).
						</p>
					</div>
				...
				<CardContent className="space-y-5">
					...
					{targetSnapshots && targetSnapshots.length > 0 && (
						<div className="rounded-lg border border-border p-3 text-xs text-muted-foreground">
							{(() => {
								const liveRunning = targetSnapshots.filter((entry) =>
									["running", "waiting"].includes(
										String(entry.live?.status || "").toLowerCase(),
									),
								).length;
								const liveQueued = targetSnapshots.filter((entry) =>
									["queued", "pending"].includes(
										String(entry.live?.status || "").toLowerCase(),
									),
								).length;
								const lastGood = targetSnapshots.filter(
									(entry) => entry.snapshot_source === "last_good",
								).length;
								return (
									<div className="flex flex-wrap items-center gap-2">
										<Badge variant="outline">
											Target scope: {targetSnapshots.length}
										</Badge>
										<Badge variant="outline">Live running: {liveRunning}</Badge>
										<Badge variant="outline">Live queued: {liveQueued}</Badge>
										<Badge variant="secondary">
											Last-good fallback: {lastGood}
										</Badge>
									</div>
								);
							})()}
					...
					<Card className="border-border">
						<CardHeader className="pb-3">
							...
							<p className="text-xs text-muted-foreground">
								Vista rapida per asset in scope: sotto ogni dominio/IP trovi le
							...
							{assetPortList.map((asset) => (
								...
								>
									<div className="flex items-start justify-between gap-3 flex-wrap">
										...
										<Badge
											variant={asset.ports.length > 0 ? "secondary" : "outline"}
										>
									...
									{asset.ports.length > 0 ? (
										<div className="mt-3 flex flex-wrap gap-2">
											{asset.ports.map((portRow, index) => {
												...
												return (
													...
													>
														<Badge
															className={exposureSeverityClass(
																portRow.exposure_level,
															)}
														>
															{portRow.port}/
															{String(portRow.protocol || "tcp").toLowerCase()}
														</Badge>
										...
										<p className="mt-3 text-xs text-muted-foreground">
											Nessuna porta aperta rilevata su questo asset nell’ultimo
											snapshot valido.
										</p>
									)}
								</div>
							...
							<CardHeader className="pb-3">
								<div className="flex items-center justify-between gap-2">
									<CardTitle className="text-base">
										Controlli avanzati Exposure (Admin)
									</CardTitle>
									<Button
										variant="outline"
										size="sm"
								...
								<CardContent className="space-y-4">
									<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
										...
										<div className="space-y-2">
											...
											<Input
												...
												placeholder="top1000 oppure 22,80,443,8443"
											/>
										</div>
											...
											<div className="rounded-md border border-border p-2 space-y-2">
												<div className="flex flex-wrap gap-1.5">
													...
													{scopePublicIps.map((ip) => (
														<Badge key={`scope-ip-${ip}`} variant="outline">
															{ip}
														</Badge>
													))}
													{scopeDomains.length === 0 &&
														scopePublicIps.length === 0 && (
															<span className="text-xs text-muted-foreground">
																Nessun target in scope.
															</span>
														)}
												</div>
											</div>
											<p className="text-xs text-muted-foreground">
												La scansione include sempre tutti i domini e IP in scope
												con controlli completi.
											</p>
										</div>
									</div>
									...
									<div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-sm">
										<label className="flex items-center gap-2">
											<Checkbox
												checked={fullControls.includeSubDiscovery}
												disabled
											/>
											Subdomain discovery
										</label>
										<label className="flex items-center gap-2">
											<Checkbox
												checked={fullControls.includePortScan}
												disabled
											/>
											Port scan
										</label>
										<label className="flex items-center gap-2">
											<Checkbox
												checked={fullControls.includeWebTech}
												disabled
											/>
											Web technologies
										</label>
										<label className="flex items-center gap-2">
											<Checkbox checked={fullControls.includeSsl} disabled />
											SSL/TLS
										</label>
										<label className="flex items-center gap-2">
											<Checkbox
												checked={fullControls.includeNetworkVuln}
												disabled
											/>
											Network vulnerability scan
										</label>
										<label className="flex items-center gap-2">
											<Checkbox checked={fullControls.detectOs} disabled />
											OS detection
										</label>
										<label className="flex items-center gap-2">
											<Checkbox
												checked={fullControls.detectServiceVersion}
												disabled
											/>
											Service version
										</label>
										<label className="flex items-center gap-2">
											<Checkbox checked={fullControls.checkAlive} disabled />
											Check alive
										</label>
										<label className="flex items-center gap-2">
											<Checkbox checked={fullControls.traceroute} disabled />
											Traceroute
										</label>
									</div>
									...
									<div className="flex flex-wrap items-center gap-2">
										...
										<Button
											...
											disabled={!selectedJobId || polling}
										>
									...
									{selectedJob && (
										<div className="rounded-lg border border-border p-3 text-sm flex flex-wrap items-center gap-2">
											...
											<Badge variant="outline">
												Stato: {statusLabel(String(selectedJob.status || ""))}
											</Badge>
											<Badge variant="outline">
												Progress:{" "}
												{statusProgress(String(selectedJob.status || ""))}%
											</Badge>
										</div>
									)}
									...
									{scopeDomains.length === 0 && (
										<p className="text-xs text-amber-300">
											Nessun dominio disponibile nello scope monitorato:
											aggiungi prima una regola di tipo dominio.
										</p>
									)}
								</CardContent>
							)}
