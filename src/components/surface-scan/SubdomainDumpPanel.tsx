...
import { Globe, Search, Loader2, Plus, Settings, AlertTriangle } from 'lucide-react';
import { toast } from 'sonner';
import { useSubdomainDump, type SubdomainResult } from '@/hooks/useSubdomainDump';
...
export const SubdomainDumpPanel: React.FC<Props> = ({ isAdmin }) => {
  const dump = useSubdomainDump();
  const { addRule } = useSurfaceScanMonitoredIps();
  const [domain, setDomain] = useState('');
  const [settingsOpen, setSettingsOpen] = useState(false);
  const [depthDraft, setDepthDraft] = useState<number>(dump.depthSetting);
  const [enabledDraft, setEnabledDraft] = useState<boolean>(dump.enabledSetting);

  React.useEffect(() => { setDepthDraft(dump.depthSetting); }, [dump.depthSetting]);
  React.useEffect(() => { setEnabledDraft(dump.enabledSetting); }, [dump.enabledSetting]);
  ...
  const handleRun = async () => {
    ...
    if (!d || !/^([a-z0-9-]+\.)+[a-z]{2,}$/i.test(d)) {
      toast.error('Dominio non valido', { description: 'Inserisci un dominio radice tipo esempio.com' });
      return;
    }
    if (!dump.enabledSetting) {
      toast.error('Subdomain Dump disabilitato', { description: 'L\'admin ha disattivato questa funzione.' });
      return;
    }
  ...
  const handleSaveSettings = async () => {
    ...
    if (ok) {
      toast.success('Impostazioni salvate', { description: `Profondità: ${depthDraft} · ${enabledDraft ? 'Attivo' : 'Disattivato'}` });
        ...
        <div className="flex items-start justify-between gap-3 flex-wrap">
          <div className="min-w-0">
            <CardTitle className="flex items-center gap-2">
              ...
              Subdomain Discovery
              {!dump.enabledSetting && (
                <Badge variant="outline" className="text-destructive border-destructive/40">disabilitato</Badge>
              )}
            </CardTitle>
            <p className="text-xs text-muted-foreground mt-1">
              Scopre sottodomini di un dominio radice tramite Certificate Transparency, Passive DNS e fonti pubbliche.
              Risultati arricchiti con IP, ASN, organizzazione e Paese. Limite massimo: <strong>{dump.depthSetting} sottodomini</strong>
              {dump.depthSetting <= 10 && ' (riduce il rumore da shared hosting)'}.
            </p>
          </div>
          {isAdmin && (
      ...
      <CardContent className="space-y-4">
            ...
            <CollapsibleContent>
              <div className="rounded-lg border border-primary/20 bg-primary/5 p-4 space-y-4 mb-2">
                <div className="flex items-center justify-between">
                  <div>
                    <Label className="text-sm font-medium">Subdomain Dump attivo</Label>
                    <p className="text-xs text-muted-foreground">Disattivalo per bloccare le scoperte per questa organizzazione</p>
                  </div>
                  <Switch checked={enabledDraft} onCheckedChange={setEnabledDraft} />
                </div>
                <div>
                  <div className="flex items-center justify-between mb-2">
                    <Label className="text-sm font-medium">Profondità massima (sottodomini per dump)</Label>
                  ...
                  <p className="text-xs text-muted-foreground mt-2">
                    Default consigliato: <strong>10</strong>. Oltre i 10 risultati il rischio di shared hosting / CDN che inquina la scansione aumenta.
                  </p>
                </div>
                <div className="flex justify-end">
                  <Button size="sm" onClick={handleSaveSettings}>Salva</Button>
                </div>
              </div>
            </CollapsibleContent>
          </Collapsible>
        ...
        <div className="flex gap-2">
          <Input
            ...
            className="flex-1"
          />
          <Button onClick={handleRun} disabled={dump.running || !dump.enabledSetting || !domain.trim()}>
            {dump.running ? <Loader2 className="w-4 h-4 mr-2 animate-spin" /> : <Search className="w-4 h-4 mr-2" />}
                ...
                <TableBody>
                  {latest.results.map((r) => (
                    <TableRow key={r.subdomain}>
                      ...
                      <TableCell className="text-xs">{r.asn ? `AS${r.asn}` : '—'}</TableCell>
                      <TableCell className="text-xs truncate max-w-[200px]">{r.asn_name ?? '—'}</TableCell>
                      ...
                      <TableCell className="text-right">
                        <Button size="sm" variant="outline" onClick={() => handleAddToMonitoring(r)}>
        ...
        {!latest && !dump.running && (
          <div className="text-sm text-muted-foreground p-4 text-center border border-dashed rounded-lg">
            Nessun dump eseguito. Inserisci un dominio radice per scoprire i sottodomini esposti.
          </div>
        )}
      </CardContent>
    </Card>
