...
export const GanttChart: React.FC<GanttChartProps> = ({
	...
	const getTimelineWidth = useCallback(
		() => timelineRef.current?.offsetWidth ?? 1,
		[],
	);
	...
	const getBarStyle = useCallback(
		(task: GanttTask) => {
			...
			const duration = Math.max(
				1,
				differenceInCalendarDays(normalizedEnd, rawStart) + 1,
			);
													...
													<Tooltip>
														<TooltipTrigger asChild>
															<div
																...
																onPointerDown={
																	canEdit
																		? (e) =>
																				handleBarPointerDown(e, task, "middle")
																		: undefined
																}
															>
																{task.progress > 0 && (
														...
														>
															...
															<p className="text-muted-foreground">
																{format(
																	parseISO(
																		liveDates[task.id]?.s || task.startDate,
																	),
																	"dd MMM",
																	{ locale: it },
																)}{" "}
																...
																{format(
																	parseISO(
																		liveDates[task.id]?.e || task.endDate,
																	),
																	"dd MMM yyyy",
																	{ locale: it },
																)}
