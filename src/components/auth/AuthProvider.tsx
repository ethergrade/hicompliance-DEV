...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({
	...
	const refreshCapabilities = useCallback(async (groupId: string) => {
		try {
			...
			/* ignora — capabilities rimangono invariate */
		}
	}, []);
	...
	useEffect(() => {
		...
		if (!token || isTokenExpired()) {
			...
			if (
				...
				!window.location.pathname.startsWith("/hiconsole") &&
				!window.location.pathname.startsWith("/reset-password") &&
				window.location.pathname !== "/saml-callback" &&
				window.location.pathname !== "/login" &&
				window.location.pathname !== "/entra"
			) {
		...
		async (loggedUser: LoginUser) => {
			...
			try {
				...
				const me = await authApi.me(
					firstGroupId && !loggedUser.is_super_admin ? firstGroupId : undefined,
				);
				setUser(me);
				setCapabilities(me.capabilities);
			} catch {
	...
	const completeMfaVerify = useCallback(
		async (challengeToken: string, code: string) => {
			try {
				const { user: loggedUser } = await authApi.mfaVerify({
				...
				const message =
					error instanceof ApiError ? error.message : "Codice non valido";
				...
				if (error instanceof ApiError && error.status === 401) {
					...
					toast({
						title: "Sessione scaduta",
						description: "Rieffettua il login",
						variant: "destructive",
					});
				} else {
					toast({
