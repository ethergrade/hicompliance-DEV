// Variante prop-driven per l'ANTEPRIMA darkrisk-preview: nessuna chiamata Supabase,
// riceve gli hit come prop (dati mock). Originale: src/components/dark-risk (branch PRODOTTO@a4145d1).
...
export function DarkRiskCredentialLeaks({ hits, limit = 100 }: Props) {
  const [showPasswords, setShowPasswords] = useState(false);
  const [search, setSearch] = useState('');
  const [copied, setCopied] = useState<string | null>(null);

  const capped = hits.slice(0, limit);
  const filtered = capped.filter((h) => {
  ...
  const copyRow = async (email: string, pwd: string) => {
    try {
      ...
      /* clipboard non disponibile in anteprima */
    }
    setCopied(email);
      ...
      <CardHeader className="pb-3">
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm font-medium flex items-center gap-2">
            ...
            Credenziali Esposte
            <Badge variant="destructive" className="text-xs">{capped.length}</Badge>
          </CardTitle>
          <Button
            variant="ghost"
            size="sm"
