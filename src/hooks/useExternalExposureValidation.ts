import { useMutation, useQuery, useQueryClient } from '@tanstack/react-query';
import { useClientOrganization } from '@/hooks/useClientOrganization';

export type ScanProfile = 'recon_safe' | 'cve_web' | 'cve_network';
export type ScanJobStatus = 'queued' | 'running' | 'partial' | 'completed' | 'failed' | 'cancelled';

export interface ExternalScanJob {
  id: string;
  organization_id: string;
  target: string;
  target_type: string;
  hosting_context: string;
  shodan_status: string;
  scan_profile: ScanProfile;
  status: ScanJobStatus;
  triggered_by: string;
  created_at: string;
  completed_at: string | null;
  error_message: string | null;
}

export interface ExternalCveFinding {
  id: string;
  scan_job_id: string;
  organization_id: string;
  target: string;
  affected_url: string | null;
  ip: string | null;
  port: number | null;
  service: string | null;
  name: string;
  cve: string[];
  cwe: string | null;
  cvss: number | null;
  cvssv3: number | null;
  epss_score: number | null;
  in_cisa_catalog: boolean;
  risk_level: number;
  severity: 'info' | 'low' | 'medium' | 'high' | 'critical';
  attribution_confidence: 'high' | 'medium' | 'low';
  confidence: 'validated' | 'active_scan_validated' | 'external_signal_not_attributed' | 'unvalidated';
  status: string;
  recommendation: string | null;
  evidence: any;
  raw_finding: any;
  created_at: string;
}

export const useExternalScanJobs = () => {
  const { organizationId } = useClientOrganization();
  return useQuery<ExternalScanJob[]>({
    queryKey: ['external-scan-jobs', organizationId],
    enabled: !!organizationId,
    refetchInterval: (q) => {
      const data = q.state.data as ExternalScanJob[] | undefined;
      return data?.some((j) => ['queued', 'running'].includes(j.status)) ? 30000 : false;
    },
    queryFn: async () => [] as ExternalScanJob[],
  });
};

export const useExternalCveFindings = (jobId?: string) => {
  const { organizationId } = useClientOrganization();
  return useQuery<ExternalCveFinding[]>({
    queryKey: ['external-cve-findings', organizationId, jobId],
    enabled: !!organizationId,
    refetchInterval: 30000,
    queryFn: async () => [] as ExternalCveFinding[],
  });
};

export interface TriggerScanInput {
  target: string;
  profile: ScanProfile;
  authorization_proof?: string;
  technologies?: string[];
  resolved_ips?: string[];
}

export const useTriggerExternalExposureScan = () => {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: async (_input: TriggerScanInput) => {
      throw new Error('Motore di validazione attiva non ancora migrato al backend API');
    },
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['external-scan-jobs'] });
    },
  });
};
